...
public class Response {
   ...
   */
  protected final List<EvalResult> evalResults = new ArrayList<>();

  /**
   * Performance data
   */
  protected final List<PerfDataResult> perfDataValues = new ArrayList<>();
  ...
  protected String getSummaryText() {
    ...
    for (final Status statusValue : Status.values()) {
      final List<String> nameList = getEvalNamesWithStatus(statusValue);
      if (!nameList.isEmpty()) {
        builder.append(", ");
        builder.append(nameList.size());
        builder.append(" ");
        builder.append(statusValue.toString().toLowerCase());

        if (!statusValue.equals(Status.OK)) {
          builder.append(" ");
